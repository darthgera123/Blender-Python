...
def run(args):
    ...
    # for i in range(0,56):
    json = load_json('/CT/prithvi/static00/nerf_synthetic/lego/transforms_test.json')
    N = len(json['frames'])
    lights = '/CT/prithvi/static00/synth_light_stage/lights/'
    all_lights = os.listdir(lights)
    for l in all_lights[:1]:
        light = load_json(os.path.join(lights,l))
        light_obj = add_light_point(xyz=light['position'], name=light['name'], size=10,energy=500)
    
    for i in range(N):
        pose = np.asarray(json['frames'][i]['transform_matrix'])
        focal_length_mm = 32 / (2 * np.tan(json['camera_angle_x'] / 2))
        cam_name = f'view_{str(i+1).zfill(2)}'
    
        cam = add_camera(xyz=(0, 0, 0),name=cam_name, rot_vec_rad=(0, 0, 0), proj_model='PERSP',sensor_height=32,sensor_width=32,f=focal_length_mm)
        ...
        if args.rgb_video:
            ...
            outdir = join(args.output_path,f'test/')
            os.makedirs(outdir,exist_ok=True)
            rgb_camspc_f = join(outdir, f'{str(i+1).zfill(3)}.png')
            render(rgb_camspc_f)
        if args.visibility:
            visible_dir = join(args.output_path,'visibility',f'{cam_name}')
            os.makedirs(visible_dir,exist_ok=True)
            visible = join(visible_dir, f'{str(i+1).zfill(3)}.png')

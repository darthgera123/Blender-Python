...
from imageio.v2 import imread,imwrite,mimsave
def topng(exr):
...
if __name__ == '__main__':
    ...
    """
    images = []
    gif_name = args.output_dir+'.gif'
    # frame_duration = 100
    frame_duration = 0.1
    # for cam in tqdm(cam_list):
    #     for j,light in enumerate(tqdm(light_list)):
    #         for i in range(0,1):
    #             path = os.path.join(args.input_dir,f'{cam}_{light}',str(i).zfill(6)+'.exr')
    #             # image = Image.open(path)
    #             # images.append(image)
    #             images.append(topng(imread(path)))
    # for cam in tqdm(range(1,56)):
    for i in range(1,96):
        # path = os.path.join(args.input_dir,f'{cam}_L{str(i).zfill(3)}',str(0).zfill(6)+'.png')
        path = os.path.join(args.input_dir,str(i).zfill(3)+'.png')
        # image = Image.open(path)
        # images.append(image)
        # images.append(topng(imread(path)))

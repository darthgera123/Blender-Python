...
if __name__ == '__main__':
    ...
    # light_list = sorted(os.listdir(args.lights))[:1]
    # for i in range(1):
    ...
    # final['view_train'] = [14]
    # final['view_test'] = [14]
    # final['view_test'] = all_views[15:]
    # final['view_train'] = all_views[:15]
    
    
    final['view_train'] = [6,7,10,15,16,17]
    final['view_test'] = [14]
    final['K'] = K
    ...
    center = (min_vertices + max_vertices) / 2.0
    print(center)
    if (center-np.asarray([0,0,0]) > 0).all():
        print("not centered")
    else:
        print("centered")
    # poses[:, :3, 3] *= scale

...
def run(args):
    ...
    easyset(n_samples=512,color_mode='RGB',h=512,w=512)


    # initialize
    bpy.context.scene.frame_end = 1
    bpy.context.scene.frame_set(1)
    for id in range(22,32):
    # cam_id = args.cam_id
        cam_id = id
        obj_params = load_json('/CT/prithvi/work/test/psnerf/dataset/armadillo/params.json')
        K = obj_params['K']
        pose = np.asarray(obj_params['pose_c2w'][cam_id])
        sensor_height = 20
        focal_length_mm = K[0][0] * 20 / 512
        cam_name = f'view_{str(cam_id+1).zfill(2)}'
        ...
        lights_path = '/CT/prithvi/static00/synth_light_stage/lights/'
        # all_lights = sorted(os.listdir(lights_path))
        # lights = obj_params['light_direction']
        # all_lights = [[ 0.,-0.731,1.462],[0.5311, -0.731, 1.462],[-0.5311, -0.731, 1.462]]
        # all_lights = [[-0.5311, -0.731, 1.462],[-0.1825, -0.7841, 1.5126000000000002]]
        lighty = load_json('/CT/prithvi/work/test/psnerf/dataset/claudia_lightstage/params.json')['light_direction']

        camera_matrix = cam.matrix_world
        camera_matrix_4x4 = camera_matrix.to_4x4()
        # camera_matrix_inv = camera_matrix_4x4.inverted()
        for num,l in enumerate(lighty[:25]):
            # lpose = np.asarray(camera_matrix)[:3,:3]@l
            # light_obj = add_light_point(xyz=lpose, size=0.3,energy=8)  
            light_name = f'L{str(num).zfill(3)}'
            # light = load_json(os.path.join(lights_path,l))
            light_obj = add_light_point(xyz=l, name=light_name, size=0.1,energy=8)
            if args.rgb_video:
                ...
                # rgb_camspc_f = join(outdir, f'{str(num+1).zfill(3)}.png')
            if args.rgb_exr:
                outdir = join(args.output_path,f'img_exr/{cam_name}')
                os.makedirs(outdir,exist_ok=True)
                rgb_camspc_f = join(outdir, f'{str(num+1).zfill(3)}.exr')
                render(rgb_camspc_f)
            if args.visibility:
        ...
        # lighty = load_json('/CT/prithvi/work/test/psnerf/dataset/claudia_lightstage/params.json')['light_direction']
        # light_name = f'L{str(num).zfill(3)}'
        # light_obj = add_light_point(xyz=all_lights[0], name=light_name, size=0.1,color=(0,1,1),energy=8)
        # light_name = f'L{str(num+1).zfill(3)}'
        # light_obj = add_light_point(xyz=all_lights[1], name=light_name, size=0.1,color = (1,1,0),energy=8)
        # for num,l in enumerate(lighty):
        #     light_name = f'L{str(num).zfill(3)}'
        #     light_obj = add_light_point(xyz=l,name=light_name, size=5,energy=1)
        # if args.rgb_video:
        #     outdir = join(args.output_path,f'img/{cam_name}')
        #     os.makedirs(outdir,exist_ok=True)
        #     rgb_camspc_f = join(outdir, f'{str(num+1).zfill(3)}.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--cam_id', type=int,  help="path to the camera .json")
    parser.add_argument(
    ...
    parser.add_argument(
        '--rgb_exr', action='store_true',  help="save_rgb_video")
    parser.add_argument(
